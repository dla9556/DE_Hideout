# D&E 아웃게임 구조도

> 기준일: 2026-03-11 | 기획서: `Dosc/design-spec.md`
> `[미구현]` 태그 = 현재 스프린트 미포함, UI만 배치

---

## 1. 메인 순환 루프

```mermaid
flowchart TD
    LOGIN([로그인]) --> CHAR_SEL

    CHAR_SEL["캐릭터 선택\n────────────\n게임 시작 시 선택\n남성 / 여성 캐릭터"]
    CHAR_SEL -->|선택 완료| HUB

    HUB["🏠 메인 허브 (은거지)\n─────────────────\nZONE-A: 상단 탭 바\nZONE-B: 좌측 패널\nZONE-C: 3D 캐릭터 (배경 씬)\nZONE-D: 우측 패널\nZONE-E: 하단 탭 바"]

    HUB -->|교환소 탭| MERCHANT["교환소 (HUD 오버레이)"]
    HUB -->|성장 탭| GROWTH["성장 (HUD 오버레이)"]
    HUB -->|성장 트리 탭| GROWTHTREE["성장 트리 (HUD 오버레이)"]
    HUB -->|ZONE-E 보관소| LOADOUT["보관소 (HUD 오버레이)"]
    HUB -->|공방 탭| WORKSHOP["공방 (HUD 오버레이)"]
    HUB -->|시스템 탭| SETTINGS["시스템 (HUD 오버레이)"]
    HUB -->|플레이 버튼| PLAY_FLOW

    subgraph PLAY_FLOW["⚔️ 플레이 진입 플로우"]
        DUNGEON[던전 선택 오버레이\n지역 맵 + 핀 방식] --> DUNGEON_DETAIL[던전 상세 패널]
        DUNGEON_DETAIL --> LOADOUT_SELECT[로드아웃 선택\n무료 / 커스텀]
        LOADOUT_SELECT --> MM_MODAL[매치메이킹 모달\nPvEvP / PvE 전용]
        MM_MODAL --> MM_SEARCH["매치메이킹 진행 중\n스피너 + mm:ss 카운터"]
        MM_SEARCH -->|매치 성공| LOADING[로딩 화면]
        LOADING --> INGAME([인게임])
        MM_SEARCH -->|취소| DUNGEON
        MM_SEARCH -->|타임아웃 120s| HUB
    end

    INGAME -->|라운드 종료| RESULT["결과창\n획득 아이템 · 경험치 요약\n보관소 이동 · 복귀"]
    RESULT -->|복귀 버튼 또는 탈출 성공| HUB
    RESULT -->|탈출 실패 · 소지품 소실| HUB
    MERCHANT --> HUB
    GROWTH --> HUB
    GROWTHTREE --> HUB
    LOADOUT --> HUB
    WORKSHOP --> HUB
    SETTINGS --> HUB
```

### HUD 오버레이 구조

메인 허브의 ZONE-C(3D 배경 씬) 위에 HUD 레이어가 오버레이됩니다. 인게임을 제외한 모든 상세 화면은 **씬 전환 없이 HUD 레이어로 표시**됩니다.

| 화면 | 진입 방법 | 구현 방식 |
|------|----------|----------|
| 교환소 | ZONE-A 교환소 탭 | HUD 오버레이 |
| 성장 | ZONE-A 성장 탭 | HUD 오버레이 |
| 성장 트리 | ZONE-A 성장 트리 탭 / 닉네임+Lv 클릭 | HUD 오버레이 |
| 보관소 | ZONE-E 보관소 버튼 | HUD 오버레이 |
| 공방 | ZONE-A 공방 탭 | HUD 오버레이 |
| 시스템 | ZONE-A 시스템 탭 | HUD 오버레이 |
| 인게임 | 매치 성공 후 로딩 | 씬 전환 |

HUD 탭은 서로 **직접 이동 가능** (탭 간 이동 시 허브로 복귀 없이 바로 전환)

HUD 헤더 오른쪽에 **재화 보유량(실버 · D 토큰 · 계곡의 인장) 상시 표시**, 각 재화 호버 시 설명 툴팁

---

## 2. Zone별 메뉴 세부 항목

```mermaid
flowchart LR
    HUB["메인 허브"]

    subgraph ZONE_A["ZONE-A  상단 탭 바"]
        A1["은거지 아이콘\n→ 허브 메인"]
        A2["닉네임 + Lv\n→ 성장 트리 (HUD 오버레이)"]
        A3["퀘스트 [미구현]\n→ 안내 메시지"]
        A4["공방\n→ 공방 (HUD 오버레이)"]
        A5["교환소\n→ 교환소 (HUD 오버레이)"]
        A6["성장\n→ 성장 (HUD 오버레이)"]
        A6B["성장 트리\n→ 성장 트리 (HUD 오버레이)"]
        A7["우편함 [미구현]\n→ 안내 메시지"]
        A7B["용병 [미구현]\n→ 안내 메시지"]
        A7C["소식 [미구현]\n→ 안내 메시지"]
        A8["시스템\n→ 시스템 (HUD 오버레이)"]
        A9["재화 (실버 · D 토큰 · 계곡의 인장)\n호버: 재화 설명 툴팁"]
        A10["서버 상태\n호버: 지역 서버 목록"]
    end

    subgraph ZONE_B["ZONE-B  중앙 좌측 패널"]
        B1["퀘스트 추적기 [미구현]\n진행 중인 퀘스트 목록"]
        B2["채팅 위젯 [미구현]\n형태 미정"]
    end

    subgraph ZONE_C["ZONE-C  중앙 3D 씬"]
        C1["3D 캐릭터 렌더\n화면을 채우는 크기\n좌우 파티원: 중앙 대비 90%"]
        C2["닉네임 레이블\n캐릭터 렌더 영역 상단 표기\n내 캐릭터: 14px #FFFFFF\n파티원: 12px #7EC8E3"]
        C3["Lv 표기\n닉네임 하단"]
        C4["파티장 👑 아이콘\nisLeader === true"]
    end

    subgraph ZONE_D["ZONE-D  중앙 우측 패널"]
        D1["스쿼드 설정 위젯\nON / OFF 토글 · 최대 3인\n랜덤 스쿼드 ON/OFF 포함"]
        D2["플레이 버튼\n→ 지역 맵 던전 선택 오버레이"]
    end

    subgraph ZONE_E["ZONE-E  하단 탭 바 (항상 표시, z-index 최상위)"]
        E1["보관소\n→ 보관소 (HUD 오버레이)"]
        E2["캐릭터 변경\n→ 캐릭터 선택 화면"]
        E3["친구 [미구현]\n→ 안내 메시지"]
    end

    HUB --- ZONE_A
    HUB --- ZONE_B
    HUB --- ZONE_C
    HUB --- ZONE_D
    HUB --- ZONE_E
```

---

## 3. 플레이 진입 서브플로우 상세

```mermaid
flowchart TD
    D2["ZONE-D: 플레이 버튼 클릭"]
    --> OV["던전 선택 오버레이\n지역 맵 (핀 방식)\nFade-In 0.3s"]

    OV --> PIN["던전 핀 클릭\n위험도별 색상 구분\nLOW/MED/HIGH/EXTREME"]
    PIN --> DETAIL["던전 상세 정보 패널\n─────────────────────\n• 던전명\n• 위험도 (LOW/MED/HIGH/EXTREME)\n• 현재 서버 인원 (범위, 익명)\n• 특수 이벤트 여부\n• 권장 장비 점수"]

    DETAIL -->|이 던전으로 선택| LS["로드아웃 선택 화면\n─────────────────\n• 무료 로드아웃\n• 커스텀 로드아웃"]

    LS --> MMM["매치메이킹 모달\n─────────────────────\n• PvEvP (기본)\n• PvE 전용"]

    MMM --> CHECK{"파티원\n준비 확인"}
    CHECK -->|전원 READY| MMS["매치메이킹 진행 중\n─────────────────\n스피너 + mm:ss\n[취소] 버튼 상시 노출"]
    CHECK -->|미준비 인원 있음| WARN["경고: N명 미준비"]
    WARN --> MMM

    MMS -->|매치 성공| LOAD["로딩 화면"]
    MMS -->|취소 클릭| OV

    MMS -->|60초 초과| POPUP["서버 혼잡 팝업\n계속 대기 여부"]
    POPUP -->|대기| MMS
    POPUP -->|취소| OV
    MMS -->|120초 타임아웃| TOAST["토스트: 매치 실패\n→ 허브 복귀"]

    LOAD --> INGAME([인게임 진입])
```

---

## 4. 스쿼드 구성 레이아웃

```mermaid
flowchart LR
    subgraph SOLO["솔로 (1인)"]
        S_NAME["PlayerName\nLv.47"]
        S_CHAR["[3D 캐릭터\n중앙 · 화면 가득]"]
        S_NAME -.위 표시.-> S_CHAR
    end

    subgraph DUO["스쿼드 (2인)"]
        D_TM["Teammate_01\nLv.23 👑"]
        D_ME["PlayerName\nLv.47"]
        D_TC["[파티원\n좌 후방 · 90%]"]
        D_MC["[내 캐릭터\n중앙 · 100%]"]
        D_TM -.-> D_TC
        D_ME -.-> D_MC
    end

    subgraph TRIO["스쿼드 (3인)"]
        T_TM1["Teammate_01\nLv.23 👑"]
        T_ME["PlayerName\nLv.47"]
        T_TM2["Teammate_02\nLv.31"]
        T_TC1["[파티원\n좌 후방 · 90%]"]
        T_MC["[내 캐릭터\n중앙 · 100%]"]
        T_TC2["[파티원\n우 후방 · 90%]"]
        T_TM1 -.-> T_TC1
        T_ME -.-> T_MC
        T_TM2 -.-> T_TC2
    end
```

---

## 5. 결과창 구조

| 항목 | 내용 |
|------|------|
| 표시 정보 | 획득 경험치 · 획득 골드 · 생존 시간 · 획득 아이템 목록 |
| 인벤토리로 이동 | 보관함의 획득 아이템을 인벤토리로 이동하는 액션 (HUD 이동 아님) |
| 인벤토리 공간 부족 시 | 이동 불가 · 에러 토스트 표시 |
| 복귀 버튼 | 허브(은거지)로 복귀 |
| 탈출 성공 | 소지품 유지 → 허브 |
| 탈출 실패 | 소지품 소실 → 허브 |

---

## 6. 보관소 HUD 패널

진입: ZONE-E 보관소 버튼 → HUD 오버레이

### 패널 구성 순서 (좌 → 우)

| 순서 | 패널 | 너비 | 설명 |
|------|------|------|------|
| 1 | 로드아웃 프리셋 | 200px | 프리셋 목록 (활성 프리셋 강조) |
| 2 | 필터링 아이콘 바 | 38px | 보관함 필터 세로 아이콘 (호버 시 라벨 우측 표시) |
| 3 | 보관함 | 270px | 기본 60칸, 확장 가능 |
| 4 | 장비 슬롯 + 배낭 패널 | flex | 상단: 장비 / 하단: 배낭 |

### 보관함 필터링 아이콘

| 아이콘 | 구분 |
|--------|------|
| 🗂 | 전체 |
| ⚔ | 무기 |
| 🛡 | 방어구 |
| 🧪 | 소모성 |
| 📜 | 레시피 |
| 📦 | 기타 |

### 보관함 확장

| 항목 | 내용 |
|------|------|
| 기본 슬롯 | 60칸 (4열, 세로 스크롤) |
| 확장 단위 | +24칸 |
| 비용 공식 | 1,000 + (확장 횟수 × 500) 골드 |
| 골드 부족 시 | 확장하기 버튼 비활성 |

### 장비 슬롯

주무기 · 보조무기 · 헬멧 · 상의 · 하의 · 장갑 · 신발 · 액세서리 · **배낭** (하단 단독 행)

### 배낭 패널

장비 슬롯 하단에 위치. 장착 배낭 종류에 따라 슬롯 수 변동.

| 배낭 종류 | 그리드 | 슬롯 수 |
|----------|--------|--------|
| 기본 배낭 | 4×2 | 8칸 |
| 중형 배낭 | 4×3 | 12칸 |
| 대형 배낭 | 4×4 | 16칸 |
| 특대형 배낭 | 4×5 | 20칸 |
| 장착 없음 | — | 0칸 |

---

## 7. 작업장 (Workshop) HUD 패널

진입: ZONE-A 작업장 탭 → HUD 오버레이 (사이드바 + 상세 구조)

### 시설 목록

| 시설 | 최대 레벨 | 기능 | 레벨업 |
|------|----------|------|--------|
| 애완슬라임 | Lv.3 | 던전 귀환 시 자원 자동 수집 | 인게임 획득 아이템 소모 |
| 무기 제작대 | Lv.5 | 무기 아이템 제작 | 인게임 획득 아이템 소모 |
| 방어구 제작대 | Lv.5 | 방어구 아이템 제작 | 인게임 획득 아이템 소모 |
| 소모품 제작대 | Lv.5 | 소모품 · 잡화 제작 | 인게임 획득 아이템 소모 |
| 레시피 보관함 | 없음 | 인게임 획득 레시피 자동 보관 | 레벨업 없음 |

### 시설 상태 흐름

```mermaid
flowchart LR
    LOCKED["미개방\n개방 비용 표시"] -->|개방하기 클릭| OPEN["Lv.1 개방됨"]
    OPEN -->|업그레이드 버튼 클릭| MODAL["업그레이드 팝업\n필요 재료 · 보유량 확인"]
    MODAL -->|재료 충분 + 업그레이드 클릭| NEXT["Lv.N+1"]
    MODAL -->|재료 부족| DISABLED["업그레이드 버튼 비활성"]
    NEXT -->|최대 레벨 도달| MAX["최대 레벨 달성"]
```

---

## 8. 예외 처리 흐름

```mermaid
flowchart TD
    EX1{"파티원 IN_GAME 상태"}
    -->|플레이 버튼 비활성화| TIP1["툴팁: 진행 중인 레이드가\n있는 파티원이 있습니다"]

    EX2{"선택 던전 점검 중"}
    -->|타일 비활성화| LABEL["빗금 오버레이\n'점검 중' 레이블"]

    EX3{"로드아웃 무기 미장착"}
    -->|경고 팝업| POPUP1["장비 없이 출격?\n확인 / 취소"]

    EX4{"보관함 공간 0%"}
    -->|경고 팝업| POPUP2["보관함 가득 참\n획득 아이템 보관 불가"]

    EX5{"네트워크 끊김"}
    -->|매치 즉시 중단| RECONNECT["재연결 시도 모달"]

    EX6{"인벤토리 공간 부족\n(결과창 이동 시)"}
    -->|에러 토스트| TOAST2["인벤토리 공간 부족\n은거지 복귀 후 정리 안내"]

    EX7{"작업장 업그레이드\n재료 부족"}
    -->|버튼 비활성| DISABLED["업그레이드 불가\n필요 재료 부족 표시"]
```

---

## [미구현] 항목 목록

| 항목 | Zone | 현재 동작 |
|------|------|----------|
| 퀘스트 탭 | ZONE-A | 클릭 시 '구현 예정 안내' 메시지 |
| 우편함 탭 | ZONE-A / HUD 오버레이 | 클릭 시 '구현 예정 안내' 메시지 |
| 퀘스트 추적기 위젯 | ZONE-B | UI 자리만 표시 |
| 채팅 위젯 | ZONE-B | UI 자리만 표시, 형태 미정 |
| 친구 탭 | ZONE-E | 클릭 시 '구현 예정 안내' 메시지 |
| 닉네임 빌보드 회전 | ZONE-C | 카메라 정면 고정 (회전 지원 여부 미결) |

---

## 변경 이력

| 날짜 | 변경 내용 |
|------|---------|
| 2026-03-10 | 캐릭터 선택 노드 추가 (메인 허브 진입 전, 남/녀 선택) |
| 2026-03-10 | 결과창 노드 추가 (인게임 → 결과창 → 메인 허브) |
| 2026-03-10 | 지역 → 던전 명칭 일괄 변경 |
| 2026-03-10 | HUD 오버레이 구조 명시 (상인·성장·성장트리·로드아웃&장비·설정) |
| 2026-03-10 | HUD 탭 간 직접 이동 가능 명시 |
| 2026-03-10 | 탈출 성공/실패 경로 인게임 → 결과창 → 메인 허브로 분리 |
| 2026-03-10 | ZONE-C 스쿼드 인원 연동 — 스쿼드 변경 시 캐릭터 표시 동적 변경 |
| 2026-03-10 | ZONE-E 항상 표시 구조 확정 (z-index 최상위, HUD/모달 위) |
| 2026-03-10 | ZONE-E 버튼 배치 변경 — 캐릭터 변경 추가, 설정 제거 |
| 2026-03-10 | ZONE-C 닉네임/레벨 위치 — 캐릭터 렌더 영역 상단 표기로 변경 |
| 2026-03-10 | ZONE-D 랜덤 스쿼드 ON/OFF 토글 추가 |
| 2026-03-10 | ZONE-C 캐릭터 크기 확대 — 화면을 채우는 크기, 좌우 파티원 90% 비율 |
| 2026-03-10 | 던전 선택 방식 변경 — 그리드 목록 → 지역 맵 핀 방식 |
| 2026-03-10 | 인게임 화면 텍스트 — "탄약/아이템" → "아이템", "(임시 화면)" 표기 추가 |
| 2026-03-10 | 결과창 — "획득한 아이템 인벤토리로 이동" + "복귀" 버튼 추가 |
| 2026-03-10 | 결과창 — 인벤토리 공간 부족 시 이동 실패 분기 추가 |
| 2026-03-10 | 작업장 HUD 패널 추가 — 5개 시설, 개별 개방/레벨업, 재료 확인 팝업 |
| 2026-03-10 | 작업장 — 애완슬라임 귀환 자원 수집 시뮬레이션 추가 |
| 2026-03-10 | ZONE-A 버튼 추가 — 성장 트리, 로드아웃 & 장비, 설정 |
| 2026-03-10 | HUD 오버레이 — 우편함(미구현) 버튼 추가, ✕ 버튼 제거 |
| 2026-03-10 | 재화 표시 — ZONE-A / HUD 오버레이 오른쪽에 골드·크리스탈 분리 표시, 호버 툴팁 |
| 2026-03-10 | 시설 상태 표기 "미개발" → "미개방" 통일 |
| 2026-03-11 | 로드아웃 & 장비 패널 전면 재구성 — 패널 순서 변경 (프리셋→필터→보관함→장비) |
| 2026-03-11 | 보관함 — 기본 60칸(4열 그리드), 확장 기능 추가 (+24칸, 골드 비용 증가 구조) |
| 2026-03-11 | 보관함 필터링 — 세로 아이콘 바 추가 (전체/무기/방어구/소모성/레시피/기타, 호버 라벨) |
| 2026-03-11 | 로드아웃 슬롯 → 로드아웃 프리셋으로 명칭 변경 |
| 2026-03-11 | 장비 슬롯에 배낭 항목 추가 (하단 단독 행) |
| 2026-03-11 | 배낭 패널 추가 — 장비 슬롯 하단 배치, 장착 배낭 종류에 따라 그리드 동적 변경 |
| 2026-03-11 | Git 저장소 초기화 및 GitHub 업로드 — https://github.com/dla9556/DE_Hideout |
| 2026-03-11 | 로드아웃 & 장비 진입 경로 변경 — ZONE-A 탭 · HUD 탭 제거, ZONE-E 인벤토리 버튼으로 단일화 |
